...
# ─────────────────────────────────────────────────────────────────────────
# PFIZER DASHBOARD — STATIC MODE
# All heavy computations (training, CV, SHAP, counterfactual, conversion
# strategy) are precomputed by precompute_data.py and stored in
# `precomputed_results.pkl`.  No XGBoost training ever happens here.
# ─────────────────────────────────────────────────────────────────────────
...
def _load_precomputed():
    """Load the precomputed pickle once per Streamlit session."""
    pkl = _Path("precomputed_results.pkl")
    if not pkl.exists():
        st.error(
            "precomputed_results.pkl not found.\n"
            "Run `python precompute_data.py` once to generate it."
        )
        st.stop()
...
# ── Thin wrappers — all return precomputed data, never train ─────────────

def load_data(dataset_name: str):
    """Static replacement — returns the raw dataframe from the pickle."""
    return _load_precomputed()["df_raw"].copy()


def train_pipeline(dataset_name: str, n_folds: int = 5):
    """Static replacement — returns the cached pipeline result dict.
    `n_folds` and `dataset_name` are accepted for API compat but ignored.
    """
    return _load_precomputed()["R"]


def run_counterfactual_scenarios(dataset_name: str):
    return _load_precomputed()["CF"]


def compute_conversion_strategy(dataset_name: str,
                                 n_candidates: int = None):
    cs = _load_precomputed()["conversion_strategy"]
    return cs["candidates_df"], cs["aggregate_df"], cs["c_medians"]


def simulate_plus1_visit_all(dataset_name: str):
    return _load_precomputed()["plus1_visit_all"]


def segment_medians(dataset_name: str, top_n: int = 6):
    sm = _load_precomputed()["segment_medians"]
    return sm["feats_full"], sm["seg_meds"]


def simulate_single_hcp(dataset_name: str, hcp_idx: int,
                         details_delta: int = 0, samples_delta: int = 0):
    """Per-HCP what-if disabled in static mode — returns the baseline
    prediction so the Doctor Explorer still renders cleanly even when the
    sliders are touched.
    """
    R = _load_precomputed()["R"]
    return (
        float(R["full"]["ord"]["P_A"][hcp_idx]),
        float(R["full"]["ord"]["P_B"][hcp_idx]),
        float(R["full"]["ord"]["P_C"][hcp_idx]),
        str(R["full"]["ord"]["pred"][hcp_idx]),
    )
...
# Page configuration
# ─────────────────────────────────────────────────────────────────────────
# Defensive load — if the favicon file is missing in a shared bundle the
# dashboard still launches; we just fall back to a generic emoji icon.
try:
    logo_pfizer = Image.open("logo2.png")
except Exception:
    try:
        logo_pfizer = Image.open("logo3.png")
    except Exception:
        logo_pfizer = "💊"
st.set_page_config(
    page_title="Pfizer Dashboard",
...
# Modeling pipeline
# ─────────────────────────────────────────────────────────────────────────
# (removed — load_data now lives in the precomputed-loader block above)
...
# (removed — train_pipeline now lives in the precomputed-loader block above)
...
# ─────────────────────────────────────────────────────────────────────────
# (removed — segment_medians now lives in the precomputed-loader block above)
...
# (removed — run_counterfactual_scenarios now lives in the precomputed-loader block above)



# (originals removed — simulate_single_hcp and compute_conversion_strategy
#  now live in the precomputed-loader block at the top of this file)
...
#    Defensive — if logo3.png isn't bundled, fall back to an SVG-free placeholder
#    so the sidebar still renders.
try:
    img_base64 = get_image_base64("logo3.png")
    _sidebar_logo_html = (
        f'<img src="data:image/png;base64,{img_base64}" width="40" '
...
# ─────────────────────────────────────────────────────────────────────────
R = train_pipeline(dataset)   # static — instant pickle load
...
# ── Probability Map (2D Ternary) ──
with tabs[2]:
    section("Probability Map (2D)",
            f"Each HCP plotted linearly by ({MODEL_LABELS_SHORT[model_pick]}) "
            "P(A) · P(B) · P(C) inside a ternary space. Color = predicted segment",
            icon="🌐")
    ...
    def _probability_map_fragment():
        ...
        # Si eligen filtrar, removemos los Unlabeled antes de graficar todo el mapa
        ...
        if highlight_row is not None:
            # Background: cada punto en gris para resaltar al HCP buscado
            df_bg = df_plot[df_plot["HCP_ID"] != highlight_row["HCP_ID"]]
            fig = go.Figure()
            fig.add_trace(go.Scatterternary(
                a=df_bg["P(C)"], b=df_bg["P(A)"], c=df_bg["P(B)"],
                mode="markers",
                marker=dict(size=4.0, color="#E2E8F0", opacity=0.6, line=dict(width=0)),
                name="Other HCPs",
                hovertemplate=("HCP %{customdata[0]}<br>"
                                "Pred %{customdata[1]} · True %{customdata[2]}<br>"
                                "P(A) %{b:.2f} · P(B) %{c:.2f} · P(C) %{a:.2f}"
                                "<extra></extra>"),
            ...
            # El punto grande seleccionado con su color correspondiente
            fig.add_trace(go.Scatterternary(
                a=[highlight_row["P(C)"]],
                b=[highlight_row["P(A)"]],
                c=[highlight_row["P(B)"]],
                mode="markers",
                marker=dict(
                    size=14, color=seg_color, opacity=1.0,
                ...
                hovertemplate=("HCP %{customdata[0]}<br>"
                                "P(A) %{b:.2f} · P(B) %{c:.2f} · P(C) %{a:.2f}"
                                "<extra></extra>"),
                customdata=[[highlight_row["HCP_ID"], highlight_row["Predicted"], highlight_row["True ATSEG"]]],
            ))
        else:
            # Mapa normal ternario 2D a color por segmento
            fig = px.scatter_ternary(
                df_plot, a="P(C)", b="P(A)", c="P(B)",
                color=color_col,
                color_discrete_map=SEG_COLORS,
                opacity=0.6,
        ...
        _style_fig(fig, height=650)
    
        fig.for_each_trace(lambda t: t.update(name=t.name + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;") if t.name else None)

        # Estilos del contenedor Ternario en 2D plano
        axis_title_font = dict(color="#475569", size=13, family="Inter, sans-serif")
        fig.update_layout(
            title=dict(text=""),
            ...
            legend=dict(
                ...
                borderwidth=0,
                font=dict(size=12, color="#0B1B33", family="Inter, sans-serif"),
                itemsizing="constant",
            ),
            ternary=dict(
                sum=1.0,
                aaxis=dict(title=dict(text="P(C)", font=axis_title_font), min=0, linewidth=1, gridcolor="rgba(0,0,0,0.06)"),
                baxis=dict(title=dict(text="P(A)", font=axis_title_font), min=0, linewidth=1, gridcolor="rgba(0,0,0,0.06)"),
                caxis=dict(title=dict(text="P(B)", font=axis_title_font), min=0, linewidth=1, gridcolor="rgba(0,0,0,0.06)"),
                bgcolor="rgba(248,250,252,0.5)"
            ),
        )

        if highlight_row is not None:
            pc = highlight_row["P(C)"]
            pa = highlight_row["P(A)"]
            pb = highlight_row["P(B)"]
            
            # Conversión matemática exacta de Ternario a Papel 2D original
            target_x = 0.5 + (pb - pa) * 0.28
            target_y = 0.18 + pc * 0.55

            tooltip_text = (
                f"<b>HCP {highlight_row['HCP_ID']}</b><br>"
                f"Pred: <b>{highlight_row['Predicted']}</b> | True: <b>{highlight_row['True ATSEG']}</b><br>"
                f"P(A): <b>{pa:.2f}</b> | P(B): <b>{pb:.2f}</b> | P(C): <b>{pc:.2f}</b>"
            )

            # Usamos tu configuración original impecable (width, height, borderpad),
            # pero desactivamos la flecha ('showarrow=False') para evitar el bug de Plotly.
            fig.add_annotation(
                xref="paper",
                yref="paper",
                x=target_x,
                y=target_y,
                text=tooltip_text,
                showarrow=False,     # 🔥 Al no haber flecha, el cuadro flota armónicamente al lado del punto sin saltar errores.
                xshift=25,           # Pequeño empuje a la derecha del punto para que no lo tape
                yshift=10,           # Pequeño empuje hacia arriba para estilizar la tarjeta
                xanchor="left",
                yanchor="middle",
                align="left",
                bgcolor="white",
        ...
        # Detail Card inferior si hay un HCP seleccionado
        if highlight_row is not None:
            ...
            st.markdown(
                ...
                <div class="info-panel">
                    <div class="info-panel-icon"></div>
                    <div class="info-panel-content">
                        <b>HCP {highlight_row['HCP_ID']}</b> seleccionado.
        ...
        def _doctor_explorer_fragment():
            ...
            if search_q.strip():
                ...
                if len(matches) == 0:
                    ...
                    # ── Segment cards + Probability breakdown ──
                    # Unified premium layout
                    pred_class = pred_o if pred_o in VALID_LABELS else ""

                    # Color mappings for the segment accent
                    _seg_accent = {
                        "SEG_A": {"grad": "linear-gradient(135deg, #003B71, #0070BF)",
                                  "bg": "rgba(0,112,191,0.06)", "border": "rgba(0,112,191,0.18)",
                                  "text": "#0070BF", "light": "#EFF6FF", "icon": "🔵"},
                        "SEG_B": {"grad": "linear-gradient(135deg, #C2410C, #F47B20)",
                                  "bg": "rgba(244,123,32,0.06)", "border": "rgba(244,123,32,0.18)",
                                  "text": "#F47B20", "light": "#FFF7ED", "icon": "🟠"},
                        "SEG_C": {"grad": "linear-gradient(135deg, #5B21B6, #7C3F98)",
                                  "bg": "rgba(124,63,152,0.06)", "border": "rgba(124,63,152,0.18)",
                                  "text": "#7C3F98", "light": "#F5F3FF", "icon": "🟣"},
                        "Unlabeled": {"grad": "linear-gradient(135deg, #64748B, #94A3B8)",
                                      "bg": "rgba(100,116,139,0.06)", "border": "rgba(100,116,139,0.18)",
                                      "text": "#64748B", "light": "#F8FAFC", "icon": "⚪"},
                    }
                    _pred_acc = _seg_accent.get(pred_class, _seg_accent["Unlabeled"])
                    _true_acc = _seg_accent.get(true_class, _seg_accent["Unlabeled"])

                    def _premium_seg_card(card_type, seg_name, accent, extra_html=""):
                        """Build a premium segment card with accent bar + large name."""
                        return (
                            f'<div style="background:#FFFFFF;border-radius:16px;'
                            f'border:1px solid {accent["border"]};'
                            f'box-shadow:0 2px 12px rgba(0,0,0,0.04);'
                            f'overflow:hidden;transition:all .25s ease;height:100%;">'
                            # Accent top bar
                            f'<div style="height:5px;background:{accent["grad"]};">'
                            f'</div>'
                            f'<div style="padding:22px 26px 20px 26px;">'
                            # Card type label
                            f'<div style="display:flex;align-items:center;gap:8px;'
                            f'margin-bottom:16px;padding-bottom:12px;'
                            f'border-bottom:1px dashed rgba(0,0,0,0.07);">'
                            f'<div style="font-size:10px;font-weight:800;'
                            f'text-transform:uppercase;letter-spacing:1.2px;'
                            f'color:{accent["text"]};">{card_type}</div>'
                            f'</div>'
                            # Large segment name
                            f'<div style="font-size:42px;font-weight:800;'
                            f'letter-spacing:-1px;line-height:1.1;'
                            f'background:{accent["grad"]};'
                            f'-webkit-background-clip:text;'
                            f'-webkit-text-fill-color:transparent;'
                            f'margin-bottom:12px;">{seg_name}</div>'
                    ...
                    if is_labeled_hcp:
                        ...
                        if pred_o == true_class:
                            match_html = (
                                ...
                                '<span style="font-size:13px;">✓</span> Matches ATSEG</div>'
                            )
                        else:
                            match_html = (
                                '<div style="display:inline-flex;align-items:center;gap:6px;'
                                ...
                                '<span style="font-size:13px;">✗</span> Differs from ATSEG</div>'
                            )
                        ...
                        pred_footer = (
                            ...
                            f'padding:6px 14px;border-radius:999px;font-size:11px;'
                            f'font-weight:700;background:{_pred_acc["light"]};'
                            f'color:{_pred_acc["text"]};'
                            f'border:1px solid {_pred_acc["border"]};">'
                            f'{max_o*100:.1f}% confidence</div>'
                            f'{match_html}'
                            f'</div>'
                        )
                        ...
                        with col_l:
                            st.markdown(
                                _premium_seg_card("Current ATSEG", true_seg,
                                                  _true_acc, true_footer),
                                unsafe_allow_html=True)
                        with col_r:
                            st.markdown(
                                _premium_seg_card("Model Prediction", pred_o,
                                                  _pred_acc, pred_footer),
                                unsafe_allow_html=True)
                    else:
                        pred_footer = (
                            f'<div style="display:flex;align-items:center;gap:10px;'
                            ...
                            f'<div style="display:inline-flex;align-items:center;gap:6px;'
                            f'padding:6px 14px;border-radius:999px;font-size:11px;'
                            f'font-weight:700;background:#F8FAFC;color:#475569;'
                            f'border:1px solid #E2E8F0;">'
                            f'No ATSEG label</div>'
                            f'</div>'
                        )
                    ...
                    # ── Probability breakdown — integrated stacked bar ──
                    def _w(v): return max(v * 100, 0.5)
                    def _lbl(seg, v): return seg if v > 0.12 else ""

                    # Build mini probability items
                    prob_items = ""
                    for _seg, _val, _acc in [("A", pa_o, _seg_accent["SEG_A"]),
                                              ("B", pb_o, _seg_accent["SEG_B"]),
                                              ("C", pc_o, _seg_accent["SEG_C"])]:
                        prob_items += (
                            f'<div style="display:flex;align-items:center;gap:10px;">'
                            f'<div style="width:10px;height:10px;border-radius:50%;'
                            f'background:{_acc["grad"]};flex-shrink:0;"></div>'
                            f'<span style="font-size:12px;color:#64748B;'
                            f'font-weight:600;min-width:38px;">SEG_{_seg}</span>'
                            f'<span style="font-size:14px;color:#0F172A;'
                            f'font-weight:800;font-feature-settings:\'tnum\';">'
                            f'{_val*100:.1f}%</span>'
                            f'</div>'
                        )
                    ...
                    breakdown_html = (
                        f'<div style="background:#FFFFFF;border-radius:16px;'
                        f'border:1px solid #E2E8F0;box-shadow:0 2px 12px rgba(0,0,0,0.04);'
                        f'overflow:hidden;margin-top:16px;">'
                        # Header
                        f'<div style="padding:18px 26px 14px 26px;display:flex;'
                        f'align-items:center;justify-content:space-between;'
                        f'border-bottom:1px solid rgba(0,0,0,0.05);">'
                        f'<div style="font-size:13px;color:#0F172A;font-weight:700;'
                        f'letter-spacing:-0.2px;">Probability Breakdown</div>'
                        f'<div style="font-size:11px;color:#64748B;font-weight:600;'
                        f'letter-spacing:0.3px;">ORDINAL MODEL</div>'
                        f'</div>'
                        # Content
                        f'<div style="padding:18px 26px 22px 26px;">'
                        # Stacked bar
                        f'<div style="display:flex;height:40px;border-radius:10px;'
                        f'overflow:hidden;background:rgba(0,0,0,0.03);'
                        f'border:1px solid rgba(0,0,0,0.05);margin-bottom:16px;">'
                        f'<div style="width:{_w(pa_o)}%;background:linear-gradient(135deg,#003B71,#0070BF);'
                        f'display:flex;align-items:center;justify-content:center;'
                        f'color:white;font-size:12px;font-weight:700;letter-spacing:0.3px;'
                        f'transition:width .8s cubic-bezier(.16,1,.3,1);">'
                        f'{_lbl("SEG_A", pa_o)}</div>'
                        f'<div style="width:{_w(pb_o)}%;background:linear-gradient(135deg,#C2410C,#F47B20);'
                        f'display:flex;align-items:center;justify-content:center;'
                        f'color:white;font-size:12px;font-weight:700;letter-spacing:0.3px;'
                        f'transition:width .8s cubic-bezier(.16,1,.3,1);">'
                        f'{_lbl("SEG_B", pb_o)}</div>'
                        f'<div style="width:{_w(pc_o)}%;background:linear-gradient(135deg,#5B21B6,#7C3F98);'
                        f'display:flex;align-items:center;justify-content:center;'
                        f'color:white;font-size:12px;font-weight:700;letter-spacing:0.3px;'
                        f'transition:width .8s cubic-bezier(.16,1,.3,1);">'
                        f'{_lbl("SEG_C", pc_o)}</div>'
                        f'</div>'
                        # Probability legend
                    ...
                    if pred_o == "SEG_B":
                        ...
                        with c_right:
                            ...
                            def stacked_bar(label, pa, pb, pc):
                                def w(v): return max(v * 100, 0.5)
                                label_a = "SEG_A" if pa > 0.12 else ""
                                label_b = "SEG_B" if pb > 0.12 else ""
                                label_c = "SEG_C" if pc > 0.12 else ""
                                return (
                                    ...
                                    f'</div>'
                                    f'<div style="display:flex;height:36px;border-radius:10px;'
                                    f'overflow:hidden;background:rgba(0,0,0,0.03);'
                                    f'border:1px solid rgba(0,0,0,0.05);">'
                                    f'<div style="width:{w(pa)}%;background:linear-gradient(135deg,#003B71,#0070BF);'
                                    f'display:flex;align-items:center;justify-content:center;'
                                    f'color:white;font-size:11px;font-weight:700;">{label_a}</div>'
                                    f'<div style="width:{w(pb)}%;background:linear-gradient(135deg,#C2410C,#F47B20);'
                                    f'display:flex;align-items:center;justify-content:center;'
                                    f'color:white;font-size:11px;font-weight:700;">{label_b}</div>'
                                    f'<div style="width:{w(pc)}%;background:linear-gradient(135deg,#5B21B6,#7C3F98);'
                                    f'display:flex;align-items:center;justify-content:center;'
                                    f'color:white;font-size:11px;font-weight:700;">{label_c}</div>'
                                    f'</div></div>'
                                )
                            st.markdown(
