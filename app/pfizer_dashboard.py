...
def _load_precomputed():
    ...
    pkl = (_REPO_ROOT / "precomputed_results.pkl")
    if not pkl.exists():
        st.error(
...
# Constants
# ─────────────────────────────────────────────────────────────────────────
# -- Shared core: model config, thresholds & palette (single source of truth) --
from src import (
    SEED, VALID_LABELS,
    PFIZER_BLUE, PFIZER_DARK_BLUE, PFIZER_LIGHT_BLUE,
    PFIZER_ORANGE, PFIZER_GREEN, PFIZER_PURPLE, PFIZER_GRAY,
    SEG_COLORS, ORDINAL_PARAMS, ARGMAX_PARAMS,
    THR_A, THR_C, KEY_FEATURES, SEG_C_WEIGHT,
    N_SHAP_TOP, N_CONVERSION_CANDIDATES,
)
SEG_C_SAMPLE_WEIGHT = SEG_C_WEIGHT   # backward-compat alias
...
try:
    logo_pfizer = Image.open(str(_ASSETS / "logo2.png"))
except Exception:
    try:
        logo_pfizer = Image.open(str(_ASSETS / "logo3.png"))
    except Exception:
        logo_pfizer = "💊"
st.set_page_config(
...
def fit_ordinal(X_train, y_train):
    """Two-stage ordinal model (P>=B, P>=C) -- delegates to src/."""
    from src import fit_ordinal as _fit_ordinal
    return _fit_ordinal(X_train, y_train)


def predict_ordinal(m1, m2, X):
    """Decision cascade: thresholds + dominance rule -- delegates to src/."""
    from src import predict_ordinal as _predict_ordinal
    return _predict_ordinal(m1, m2, X)


def fit_argmax(X_train, y_train):
    """Balanced multiclass baseline -- delegates to src/."""
    from src import fit_argmax as _fit_argmax
    return _fit_argmax(X_train, y_train)


def predict_argmax(m, le, X):
    """Baseline prediction (argmax) -- delegates to src/."""
...
def _render_cover():
    ...
    try:
        _logo_b64 = get_image_base64(str(_ASSETS / "logo3.png"))
        _logo_html = (
...
try:
    img_base64 = get_image_base64(str(_ASSETS / "logo3.png"))
    _sidebar_logo_html = (
        ...
        if search_q.strip():
            ...
            if len(matches) == 0:
                ...
                def stacked_bar(label, pa, pb, pc):
                    """Stacked bar."""
                    def w(v):
                        """Scale a value to a clamped, non-negative percentage."""
                        return max(v * 100, 0)
                ...
                def _close(xs):
                    """Close a polygon by repeating its first point."""
                    return xs + [xs[0]]
