...
from sklearn.metrics import accuracy_score, cohen_kappa_score, f1_score, confusion_matrix, roc_curve
...
def train_one_vs_rest(labeled):
    ...
    return y_pred, y_test, seg_results, {'accuracy': acc, 'kappa': kappa, 'f1': f1}
...
def create_visualization(y_test, y_pred, seg_results, metrics):
    ...
    title = f'ONE-VS-REST + THRESHOLD OPTIMIZATION\nAcc: {metrics["accuracy"]:.1%} | F1: {metrics["f1"]:.3f} | Kappa: {metrics["kappa"]:.3f}'
